...
const agentDependencies: Record<string, { tasks: string[], workflows: string[], scripts: string[] }> = {
  'servitor': {
...
try {
  ...
  if (!existsSync(mcpDocsDir)) {
    cpSync(
      join(__dirname, '..', 'agent-tree', 'manuals', 'mcp-installation-docs'),
      mcpDocsDir,
      { recursive: true }
    );
  }
  ...
  cpSync(
    join(__dirname, '..', 'agent-tree', 'agents', agent, `${agent}.md`),
